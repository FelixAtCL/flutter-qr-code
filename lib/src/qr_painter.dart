      ...
      for (var y = 0; y < _qr!.moduleCount; y++) {
        ...
        switch (dataModuleStyle.dataModuleShape) {
          case QrDataModuleShape.square:
            ...
            break;
          case QrDataModuleShape.heart:
            final width = squareRect.width;
            final height = squareRect.height;

            final path = Path();

            path.moveTo(
              squareRect.left + 0.5 * width,
              squareRect.top + height * 0.35,
            );

            path.cubicTo(
              squareRect.left + 0.2 * width,
              squareRect.top + height * 0.1,
              squareRect.left + (-0.25 * width),
              squareRect.top + height * 0.6,
              squareRect.left + 0.5 * width,
            ...
            path.cubicTo(
              squareRect.left + 0.8 * width,
              squareRect.top + height * 0.1,
              squareRect.left + 1.25 * width,
              squareRect.top + height * 0.6,
              squareRect.left + 0.5 * width,
              squareRect.top + height,
            );
